...
def main():
    ...
    prediction = input_data.get('prediction', '00')
    ...
    try:
        ...
        # For multi-class, shap_values is 3D: (n_samples, n_features, n_classes)
        # Get SHAP for predicted class
        pred_idx = label_encoder.transform([prediction])[0]
        if len(shap_values.shape) == 3:
            feature_shap = shap_values[0, :, pred_idx]
        else:
        ...
        for i, col in enumerate(FEATURE_COLS):
            feature_importance[col] = float(feature_shap[i])
        ...
        output = {
            ...
            "base_value": float(explainer.expected_value[pred_idx] if hasattr(explainer.expected_value, '__len__') else explainer.expected_value)
        }
        
    except Exception as e:
        output = {
            "error": str(e),
            "fallback": "SHAP explanation failed"
        }
